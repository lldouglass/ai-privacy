...
\begin{itemize}[leftmargin=*]
  ...
  \item \textbf{Survey (\texttt{SurveyPage.jsx}):} Ten question nodes drive branching logic that labels systems as unacceptable, high-risk, limited, or minimal risk. Answers persist in \texttt{sessionStorage} so users can revisit prior decisions.
...
\begin{enumerate}[leftmargin=*]
  \item \textbf{Survey authoring via configuration.} All branching logic lives inside a single \texttt{surveyQuestions} array, making it easy to edit thresholds without touching React state code.
