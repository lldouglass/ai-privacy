...
# # OpenAI only required when not demo
# client = OpenAI(api_key=os.getenv("OPENAI_API_KEY") or None)
# if not client.api_key and not DEMO_MODE:
#     raise RuntimeError("Set OPENAI_API_KEY or enable DEMO_MODE=1")
...
# # ── FastAPI & CORS ────────────────────────────────────────────────────────
# app = FastAPI(title="AI Compliance Assistant", version="0.2.0")
...
# # ── Retrieval index (skip in demo) ────────────────────────────────────────
# REG_INDEX = None
# if not DEMO_MODE and build_or_load_index:
#     REG_INDEX = build_or_load_index(client)
...
#         model="gpt-4-turbo",
#         messages=[{"role": "system", "content": "Precise compliance analyst. No overclaiming."},
#                   {"role": "user", "content": prompt}],
#         temperature=0.15,
#     )
#     report_md = rsp.choices[0].message.content
#     usage = rsp.usage.model_dump() if hasattr(rsp, "usage") else None
#     total_tokens = usage.get("total_tokens") if usage else None
#     cost = round(total_tokens / 1000 * 0.03, 4) if total_tokens else None
...
# ):
#     _rate_limit(request.client.host); _check_invite(request)
#     # Parse YAML/JSON into a string block (no LLM needed for demo)
#     try:
#         raw = (await file.read()).decode()
#         meta = yaml.safe_load(raw)
#         model_meta = yaml.safe_dump(meta, sort_keys=False)  # keep YAML format
